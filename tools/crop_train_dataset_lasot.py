...
LaSOT_dir = r'E:\dataset\tracker_evaluate_dataset\LaSOT'
cropped_data_dir = r'G:\dataset\train_dataset\Cropped_LaSOT'
seqs = LaSOT(LaSOT_dir, subset='train', return_meta=True)
...
def dataset_filter(img0, anno, vis_ratios=None):
    ...
    if vis_ratios is not None:
        c8 = (vis_ratios > max(0, vis_ratios.max() * 0.3))  # 目标的可见区域是否超过最大可见区域大小的0.3倍。 注意，LaSOT只标注了是否遮挡
    else:
        c8 = np.ones_like(c1)
...
def crop_dataset(meta_exist, seq_index, cropped_data_dir):
    ...
    if meta_exist:
        ...
        full_occlusion = meta.get('full_occlusion', None)
        out_of_view = meta.get('out_of_view', None)
        vis_ratios = 1 - np.logical_or.reduce([full_occlusion, out_of_view])
        # vis_ratios = 1 - meta.get('full_occlusion', None)
    else:
    ...
    target_ratio = exemplar_sz / (np.sqrt(size_w * size_h)+1e-5)
    ...
    for image_file_index in range(len(image_files)):
        if vis_ratios[image_file_index] == 0:  # LaSOT中对于遮挡样本是没有标记的所以直接跳过存在遮挡的样本。
            print('sequence:{} frame:{} occlusion skip this frame'.format(seq_name, image_file_index))
            continue
        ...
        try:
            ...
            print('seq_name:{}  image_name:{}'.format(seq_name, image_name))
