...
def dataset_filter(img0, anno, vis_ratios=None):
    ...
    c3 = np.all(anno[:, 2:] <= 500, axis=1)  # 目标的长宽像素是否均小于50
    ...
    if vis_ratios is not None:
        c8 = (vis_ratios > max(1, vis_ratios.max() * 0.3))  # 目标的可见区域是否超过最大可见区域代销的0.3倍。
    else:
        c8 = np.ones_like(c1)
...
# def crop_dataset(meta_exist, seqs, seq_index, cropped_data_dir):
#     #  裁剪视频序列。
#     # 获取视频序列的信息。
#     if meta_exist:
...
context = 0.5
exemplar_sz = 127
got10k_dir = os.path.expanduser(r'G:\dataset\train_dataset\GOT-10K')
cropped_data_dir = r'D:\Cropped_GOT10K'
seqs = GOT10k(got10k_dir, subset='train', return_meta=True)  # 由于seqs是类，似乎使用多线程时，向子任务汇总传递类会出现未知错误，所以改成全局，使得多线程得以实现。
...
if __name__ == '__main__':
    ...
    # exemplar_sz = 127
    # got10k_dir = os.path.expanduser(r'G:\dataset\train_dataset\GOT-10K')
    # cropped_data_dir = r'G:\dataset\train_dataset\Cropped_GOT10K'
    # seqs = GOT10k(got10k_dir, subset='train', return_meta=True)
